...
|------|-------------|------|---------------|----------|-------------------------|
| `vault.search(query, type=None, k=5, rerank=False)` | Hybrid-Suche (BM25 + vec0-KNN + RRF); `rerank=True` aktiviert zusätzlich den lokalen Reranker | `vault.search("transformer attention", k=10)` | Vault mit indizierten Papers; die Vektor-Hälfte braucht ein geladenes Embedding-Modell | `list[dict]` der Treffer, nach Rang zusammengeführt — paperzentriert (ein Eintrag je `paper_id`); mit `rerank=True` trägt jeder Treffer zusätzlich die Fundstelle des besten Chunks: `section` (Abschnittstitel), `page_start`/`page_end` (Seitenbereich) — `None`, wenn der Chunk keine Lokation trägt (Bestandschunks vor Issue #728, oder Fallback-Kandidaten ohne echten Chunk) | Leere Liste trotz passender Papers — dann fehlt der Index; `vault.component_status()` nennt die Ursache |
...
| `vault.restore_snapshot(slug, ts, snapshots_dir=None, target_dir=".")` | Stellt Snapshot `<slug>/<ts>.tgz` wieder her: State-Dateien nach `target_dir`, das `vault.db`-Member an den kanonischen Vault-Pfad des Servers (nicht nach `target_dir`). Der bisherige Bestand dieser Datei wird davor automatisch als `vault.db.<YYYYMMDD-HHMMSS>.bak` daneben gesichert | `vault.restore_snapshot("mein-projekt", "20260604-0930")` | Vorhandene Datei `<slug>/<ts>.tgz` | `dict` — `{"ok", "tarball", "restored_files", "vault_db_restored", "vault_db_backup", "vault_db_skipped", "error"}`; `ok` ist nur `True`, wenn tatsächlich etwas zurückgespielt wurde. Über das MCP-Tool ist `vault_db_skipped` stets `None`, weil die Tool-Schicht den Vault-Pfad selbst auflöst und ausdrücklich übergibt | `ok: False` mit Klartext in `error`, oder `ValueError` bei Symlink, absolutem Pfad oder Path-Traversal im Archiv |