...
| `publisher-fetchers` | structural | `tests/test_publisher_fetchers.py` (Schema-Assertions) | Cases erwarten Verlagsseiten inkl. Captcha-/Auth-Pfaden; nicht hermetisch reproduzierbar. Fuer `pf-06`/`pf-07`/`pf-08` (#449, Fix-Runde PR #500) wurde der in `agents/cambridge-core.md`/`agents/oxford-academic.md`/`agents/jstor.md` dokumentierte Discovery-/Download-Flow am 29.07.2026 manuell via `browser-use` (isolierte Session, keine Zugangsdaten) nachvollzogen -- kein automatisierter Testlauf, aber echte Belege statt Behauptung: `pf-06` und `pf-07` laden real ein vollstaendiges Buch-PDF ohne Login (228 bzw. 225 Seiten; `pf-07`s urspruengliche DOI war zudem sachlich falsch und wurde korrigiert, siehe Case-Notiz); `pf-08` loest bei direktem PDF-Zugriff sofort JSTORs reCAPTCHA "Access Check" aus (Block-Referenz dokumentiert) -- bestaetigt das im Agent behauptete hohe Anti-Scraping-Risiko empirisch. Automatisierte Wiederholung bleibt unmoeglich (Live-Netz gegen fremde Verlagsseiten, kein Fixture-Ersatz) -- Status bleibt `structural`. |
...
**Bilanz:** 3 × `metric`, 42 × `structural`, 0 × `removed` (Stand Issue #446:
`word-export`/`slide-export` neu, beide `structural`).
