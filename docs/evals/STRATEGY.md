...
| `free-archive-fetchers` | structural | `tests/test_free_archive_fetchers.py` (Prompt- und Eval-Assertions), `tests/test_free_archive_download.py` (Navigations-Spiegel gegen einen lokalen HTTP-Ursprung), `tests/test_free_archive_live_fetch.py` (opt-in via `RUN_LIVE_ARCHIVE_FETCH=1`) | Die Beschaffung laeuft end-to-end: `tests/helpers/local_origin.py` serviert die live erhobene DOM der drei Archive **und** die PDF-Route auf 127.0.0.1, `tests/helpers/archive_fetcher_nav.py` faehrt den Weg des Agenten ab, laesst die Datei im Session-Download-Verzeichnis landen, verschiebt sie und verifiziert sie **von der Platte** (existiert, > 10 KB, beginnt mit `%PDF-`); der Test vergleicht die geschriebenen Bytes mit den ausgelieferten. Negativkontrollen: Borrow-only, Suche-im-Buch, HathiTrusts Rate-Limit (inkl. Wiederholungsversuch, der zwischendurch freigibt), Nicht-PDF-Antwort, Datei unter der Groessenschwelle, MDZ ohne bestaetigten Rechtehinweis. Der Status bleibt `structural`, weil die DOM aus Fixtures stammt und das oeffentliche Netz der drei Archive in CI ungetestet bleibt (netzabhaengige Laeufe wuerden CI bei fremden Ausfaellen rot faerben, gleiche Lage wie `oa-fetchers`). Der reale Stand haengt stattdessen als Artefakt daneben: `evals/free-archive-fetchers/live-verification.json` haelt je Agent einen Lauf vom 2026-07-29 mit URL-Kette, HTTP-Status, Bytes und SHA-256 fest — Internet Archive und MDZ mit verifiziertem PDF, HathiTrust als `rate_limited` (HTTP 429 auf der Download-Route; die Item-Seite selbst laedt im echten Browser vollstaendig) — nachfahrbar ueber den opt-in-Test. Der Eintrag traegt zusaetzlich eine `mechanism_probe`, die den 403 der Cloudflare-Challenge vom 429 des Rate-Limits trennt und die einschlaegige `robots.txt`-Zeile (`Disallow: /cgi/` fuer `User-agent: *`) festhaelt. |
...
**Bilanz:** 3 × `metric`, 39 × `structural`, 0 × `removed`.
...
`evals/SCHEMA.md` beschreibt das `prompts[]`-Format. Sechs Verzeichnisse benutzen
faktisch ein zweites, `cases[]`-basiertes Format (`fetch`, `figure-verifier`,
`generic-fetcher`, `oa-fetchers`, `publisher-fetchers`, `free-archive-fetchers`;
`figure-verifier`, `oa-fetchers` und `free-archive-fetchers` sogar als
Top-Level-Array ohne `component`-Feld). Das ist in `evals/SCHEMA.md`
...
`generic-fetcher` (#448) und `free-archive-fetchers` (#450) folgen demselben
`cases[]`-Format wie die verwandten Fetcher-Verzeichnisse, statt weitere
