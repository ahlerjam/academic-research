...
| `free-archive-fetchers` | structural | `tests/test_free_archive_fetchers.py` (Schema-Assertions) | Cases erwarten Live-Downloads von HathiTrust/Internet Archive/MDZ; jeder Lauf wäre netzabhängig und würde CI bei fremden Ausfällen oder Rate-Limits (HTTP 429) rot färben — gleiche Lage wie `oa-fetchers`. |
