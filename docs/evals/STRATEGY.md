...
| `topic-brainstorm` | structural | `tests/evals/test_rest_evals.py` (API-gated), `tests/evals/test_triggers.py` (API-gated), `tests/evals/test_eval_coverage.py`, `tests/evals/test_topic_brainstorm_criteria.py` (offline — prüft die Kriterien, nicht das Modell) | Ideengenerierung ist per Definition offen; ein Offline-Assert würde Vielfalt bestrafen. Die Quality-Prompts (seit #471 inklusive der Fach-Kontrast-Fälle `tb-04`/`tb-05`) hingen bis #471 an gar keinem Runner und laufen seither über `test_rest_evals.py`. Ohne Key Skip. Offline entschieden ist seit #471 allein, ob diese Kriterien überhaupt messen können: die reproduzierte Fixed-Set-Ausgabe des Bugs muss durchfallen, eine fachlich passende Antwort bestehen. Das ist eine Instrumenten-, keine Modellprüfung — der Status bleibt deshalb `structural`. |
