...
| [`2026-08-10-vault-mcp-evals-824.md`](2026-08-10-vault-mcp-evals-824.md) | Live-Nachweis, dass sich der `academic-vault`-MCP-Server an eine CLI-Eval-Sitzung binden lässt (#824): geseedete Wegwerf-Vault, PDF-Fixtures mit Text-Layer, zwei vom Agenten geschriebene `local-verbatim`-Zitate als Beleg — `--allowedTools mcp__academic-vault__*,Read` genügt, `bypassPermissions` nicht nötig |
| [`2026-08-10-vault-mcp-evals-824-live-results.json`](2026-08-10-vault-mcp-evals-824-live-results.json) | Rohdaten der Sondierung (vollständiges CLI-Envelope inklusive `permission_denials`, Turns, Kosten — plus die vom Agenten in die Wegwerf-Vault geschriebenen Zitate) |
...
| [`2026-08-11-humanizer-style-evaluator-boundary-825.md`](2026-08-11-humanizer-style-evaluator-boundary-825.md) | Trigger-Recall `humanizer-de`/`style-evaluator`: vier von sechs Missern waren ein Mess-Harness-Defekt (Klassifikations-Prompt ging als gewöhnliche User-Nachricht ein, Modell antwortete mit Rückfragen statt zu dispatchen) statt einer Beschreibungslücke; `<user_prompt>`-Kapselung als Härtung belegt (FPR 0/10 über acht Skills unverändert), revidierte humanizer-de/style-evaluator-Abgrenzung aus #177, volle Achter-Skill-Nachmessung — löst #837 ab (#825) |
