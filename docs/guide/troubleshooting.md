...
| `permission-blocked` / Browser-Teil übersprungen | Chrome-Verbindung ist gerade nicht aktiv (Klick auf „Allow" im Chrome-Popup verfällt). `~/.academic-research/venv/bin/python ${CLAUDE_PLUGIN_ROOT}/scripts/browser_connection_setup.py --setup --force` erneut ausführen, dabei den Dialog unter `chrome://inspect/#remote-debugging` bestätigen. Alternative ohne Klick (kostenpflichtig): Cloud-Browser via `browser-use auth login` |
| `--mode deep` bricht unbeaufsichtigt ab | **Voraussetzung für einen unbeaufsichtigten `--mode deep`-Lauf:** ein per `/academic-research:setup` (Schritt 4) bereits eingerichteter und aktiver Chrome-Verbindungsweg. Ohne das prüft `scripts/browser_preflight.py` vor dem ersten Browser-Modul und bricht den Browser-Teil kontrolliert ab (API-Module laufen unabhängig davon weiter) — ohne eingerichtete Verbindung ist `--mode deep` **nicht** unbeaufsichtigt lauffähig |
| „Verbindungsweg nachtragen" beim ersten Lauf nach dem Update | Normal bei Bestandsinstallationen ohne `~/.academic-research/browser_connection.json` (die Datei gibt es erst seit #907): `browser_preflight.py` meldet eine funktionierende lokale Verbindung, trägt den Weg automatisch nach und lässt den Lauf durch. Erscheint nur einmal — danach ist der Vermerk vorhanden |
