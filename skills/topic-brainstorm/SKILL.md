...
Kandidaten, die zu Studienrichtung, Interessensgebieten, Zeitbudget und
Datenzugang des Users passen — die Urteilsarbeit liegt hier, nicht in
`scorer.py`. Zwei Anfragen aus unterschiedlichen Fächern müssen zu erkennbar
unterschiedlichen, fachlich passenden Kandidaten führen.
...
- `reason`: 1-2 Sätze, **warum** das Thema zur genannten Studienrichtung,
  den Interessen und dem Zuschnitt (Budget/Datenzugang) passt — Pflichtfeld,
  nicht leer lassen
...
Schreibe das Array in eine temporäre JSON-Datei und rufe den Scorer auf
(er normalisiert Feasibility über Budget-/Datenzugang-Modifikatoren und
Novelty über den Interessens-Overlap; Career-Fit und `reason` reicht er
unverändert durch):
...
Zeige pro Topic:
- Den `reason` — warum das Thema zum Zuschnitt passt
...
- **Keine feste Themenliste** — Kandidaten fach- und interessenspassend selbst entwerfen (Schritt 2), nie aus einer vordefinierten Liste kopieren
