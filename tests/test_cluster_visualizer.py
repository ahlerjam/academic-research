...
def test_linkstyle_stroke_width_for_weight3(cluster_data):
    ...
    assert "stroke-width:6px" in src, f"linkStyle fuer weight=3 fehlt. Quelltext:\n{src}"
...
def test_label_sanitization():
    ...
    data = {
        ...
        "papers": [
            {"id": "test_paper", "title": 'Deep "Learning": A|B', "year": 2024, "citations": []}
        ],
        "edges": [],
    }
...
def test_skill_trigger_phrases():
    ...
    for phrase in ["zeige Cluster", "visualisiere", "Mindmap"]:
        assert phrase in content, f"Trigger-Phrase '{phrase}' fehlt in SKILL.md"
