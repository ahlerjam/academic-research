...
def test_skill_md_frontmatter_valid():
    ...
    assert fm.get("name") == "humanizer-de", f"Expected name='humanizer-de', got {fm.get('name')!r}"
...
def test_patterns_contains_45_patterns():
    ...
    assert len(matches) == 45, f"Expected 45 patterns (#### N. headings), found {len(matches)}"
