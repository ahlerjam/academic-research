...
def test_registered_tool_count_is_stable() -> None:
    """Sanity-Check: server.py registriert genau 46 MCP-Tools (Drift-Anker).

    Erhoeht sich, wenn neue @mcp.tool dazukommen (zuletzt +1 via #604:
    vault.check_retractions); zuletzt -1 via #632: vault.ensure_file entfiel
    mit dem Files-API-Pfad. Bei Aenderung: README-Tabellen UND diese Zahl
    gemeinsam aktualisieren.
    """
    tools = _registered_tools()
    assert len(tools) == 46, f"Erwartet 46 registrierte @mcp.tool, gefunden {len(tools)}: {tools}"
