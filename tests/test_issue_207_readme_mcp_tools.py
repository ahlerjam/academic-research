...
def test_registered_tool_count_is_stable() -> None:
    """Sanity-Check: server.py registriert genau 45 MCP-Tools (Drift-Anker).

    Erhoeht sich, wenn neue @mcp.tool dazukommen (zuletzt +3 via #630:
    vault.extract_tables, vault.list_tables und vault.get_table_cell);
    zuletzt -1 via #632: vault.ensure_file entfiel mit dem Files-API-Pfad.
    Bei Aenderung: README-Tabellen UND diese Zahl gemeinsam aktualisieren.
    """
    tools = _registered_tools()
    assert len(tools) == 45, f"Erwartet 45 registrierte @mcp.tool, gefunden {len(tools)}: {tools}"
