...
- **Fetcher-Agents für Cambridge Core, Oxford Academic und JSTOR (#449):** Drei
  ...
  `captcha` erfüllt) und nie ausgeführt wurden — `docs/evals/STRATEGY.md` führt
  `publisher-fetchers` projektweit weiterhin als `structural`. Der in den drei
  Agent-Dateien dokumentierte Discovery-/Download-Flow wurde daraufhin am
  29.07.2026 manuell via `browser-use` (isolierte Session, keine Zugangsdaten)
  nachvollzogen: Cambridge Core und Oxford Academic laden real ein
  vollständiges Buch-PDF ohne Login (228 bzw. 225 Seiten); JSTOR löst beim
  direkten PDF-Zugriff sofort ein reCAPTCHA „Access Check" aus (Block-Referenz
  dokumentiert) — bestätigt das dokumentierte hohe Anti-Scraping-Risiko
  empirisch. Dabei aufgedeckt: die ursprüngliche Oxford-Academic-Testfall-DOI
  (`10.1093/oso/9780190247249.001.0001`) referenzierte tatsächlich ein
  reguläres, bezahlpflichtiges Buch ohne jedes OA-Badge, keine
  Commit-to-Open-Publikation — korrigiert auf einen über die offizielle
  Titelliste (`academic.oup.com/commit-to-open/pages/collections`)
  verifizierten realen Titel. Alle drei Eval-Cases tragen jetzt einen
  konkreten, belegten Zielwert (`equals:success`/`equals:captcha`) statt der
  vagen Existenzprüfung; neuer Regressionstest
  `tests/test_issue_449_fetcher_evidence.py` hält Zielwerte, DOI-Korrektur und
  Verifikationsnachweis fest. Status bleibt `structural` (Live-Netz gegen
  fremde Verlagsseiten bleibt nicht hermetisch automatisierbar) — jetzt aber
  mit echtem statt nur behauptetem Beleg für die drei neuen Plattformen.
