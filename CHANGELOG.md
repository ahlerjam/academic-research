...
- **`openpyxl` fehlte als Dependency — `/excel` und `/pickup` real defekt (#367):** Der vendorierte xlsx-Skill (`skills/xlsx/scripts/recalc.py`) braucht `openpyxl` zwingend, das Paket stand aber weder in `scripts/requirements.txt` noch in `pyproject.toml` — entgegen der Behauptung in `commands/setup.md`/`commands/excel.md`. Ein frisches Setup über den dokumentierten Weg ließ beide Slash-Commands mit `ModuleNotFoundError` scheitern. Issue #235 hatte `openpyxl` zuvor bewusst entfernt, weil kein `scripts/*.py`-Modul es importiert — der damalige Scan erfasste den Konsumenten in `skills/xlsx/` nicht; `tests/test_issue_235_unused_deps.py` prüft seither nur noch `pandas`. Neuer Regressionstest `tests/test_issue_367_openpyxl_dependency.py`.
