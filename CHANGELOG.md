...
- **`voyageai`/`cohere`/`FlagEmbedding` als optionale Extras (#376):** `pyproject.toml` deklariert zwei neue `[project.optional-dependencies]`-Gruppen — `rerank-cloud` (`voyageai`, `cohere`; `uv sync --extra rerank-cloud`) und `rerank-local` (`FlagEmbedding`, `transformers<5.0`; `uv sync --extra rerank-local`), bewusst getrennt von `dev`, damit die Default-CI-Installation schlank bleibt. `transformers<5.0` ist notwendig, weil `FlagEmbedding==1.4.0` intern `Tokenizer.prepare_for_model()` aufruft, das in `transformers>=5.x` entfernt wurde (verifiziert per Live-Modell-Download, `AttributeError`) — Deckelung bis FlagEmbedding nachzieht.
