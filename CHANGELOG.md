...
- **`topic-brainstorm` fachabhängig statt hartkodierter Cyber-Security-Liste (#471):** `scripts/scorer.py` enthielt eine fest kodierte `_TOPIC_DB` mit 5 Themen ausschließlich aus dem Cyber-Security-Bereich; `_normalize_field()` mappte zusätzlich jede unbekannte Studienrichtung (z. B. „Maschinenbau", obwohl `SKILL.md` diese Option selbst anbietet) still auf „Wirtschaftsinformatik" — nur der `career_fit`-Score variierte je Fach, Titel, Forschungsfragen und Pilot-Papers blieben für jede Anfrage identisch. Folgt jetzt dem im Repo etablierten Muster von `methodology-advisor` (Scoring-Rubrik lebt im `SKILL.md`, das Modell wendet sie an — keine feste Kandidatenliste in Python): `_TOPIC_DB` und `_normalize_field`/`_FIELD_NORMALIZE` sind aus `scorer.py` entfernt, ein neuer `--topics-json <pfad|->`-Input (Datei oder stdin) nimmt die vom Modell fach- und interessenspassend entworfenen Kandidaten entgegen. `scorer.py` normalisiert nur noch Feasibility (Budget-/Datenzugang-Modifikatoren) und Novelty (Interessens-Overlap); Career-Fit und das neue Pflichtfeld `reason` (warum das Thema zum Zuschnitt passt) reicht der Scorer unverändert durch. `SKILL.md` Schritt 2 beschreibt jetzt den Kandidaten-Entwurf inkl. Beispiel-Snippet und Begründungspflicht; `references/scoring-criteria.md` stellt die Career-Fit-Referenzwerte als Orientierungshilfe fürs Modell dar statt als internen Scorer-Lookup. **Ehrlich benannte Grenze:** Dass das Modell inhaltlich sinnvolle, fachspezifische Themen erfindet, ist ein Generierungsverhalten und laut `docs/evals/STRATEGY.md` als `structural`/API-gated eingestuft — pytest beweist nur den Mechanismus (kein Fixed-DB-Fallback mehr, keine stille Feld-Normalisierung, Kandidaten werden unverändert durchgereicht und bleiben bei zwei unterschiedlichen Fach-Fixtures disjunkt), nicht die inhaltliche Themenqualität selbst; zwei neue Fach-Kontrast-Prompts in `evals/topic-brainstorm/evals.json` decken das eval-seitig ab. Da `SKILL.md` dadurch wächst, hebt `tests/baselines/skill_sizes.json`/`tokens.json` die topic-brainstorm-Baseline um den Netto-Zuwachs an (7200 → 9635 Zeichen, 1412 → 1978 Token-Proxy) — etabliertes, mehrfach genutztes Repo-Muster für legitimes Skill-Wachstum (vgl. 89ca331, d12a976, #395/PR #439). Neu bzw. umgeschrieben: `tests/test_topic_brainstorm.py` (Fixtures statt Live-DB-Erwartung, neue Fachabhängigkeits- und Reason-Tests).
- **Eval-Prompts von `topic-brainstorm` hatten keinen Ausführungspfad — plus Rücknahme eines Schein-Nachweises (#471, Fix-Runde zu PR #484):** Die Quality-Prompts `tb-01`–`tb-05` aus `evals/topic-brainstorm/evals.json` wurden von **keinem** Runner eingesammelt: `tests/evals/test_triggers.py` liest ausschließlich `trigger_evals.json`, und `tests/evals/test_rest_evals.py` sammelt nur die Komponenten aus seiner eigenen Liste — `topic-brainstorm` stand dort nicht. Die in dieser PR ergänzten Fach-Kontrast-Prompts `tb-04` (Maschinenbau/additive Fertigung) und `tb-05` (BWL/Nachhaltigkeit) lagen damit in einer toten Datei und wären auch mit gesetztem `ANTHROPIC_API_KEY` nie gelaufen. `topic-brainstorm` steht jetzt in `REST_SKILLS`; die fünf Prompts sind erstmals ausführbar, sobald ein Key vorliegt (ohne Key weiterhin Skip — Issue #55 bleibt geschlossen, kein Budget angefordert). **Zurückgenommen:** Eine zwischenzeitliche Testklasse `TestAC1ExecutedEvidenceForEvalPrompts` sollte AC1 offline belegen, war aber zirkulär — sie bewertete vom Testautor selbst geschriebene Kandidaten per `check_expected()` gegen genau die Regex, auf die diese Kandidaten geschrieben worden waren, durch einen Scorer, der `title`/`keywords`/`reason` nachweislich unverändert durchreicht. Gegenprobe (real gelaufen): ersetzt man die Maschinenbau-Kandidaten durch die fachfremden Cyber-Security-Themen der in diesem Issue gelöschten `_TOPIC_DB` und schreibt lediglich das Wort „Maschinenbau" in jede `reason`, bleiben alle fünf Tests grün — der Nachweis konnte genau den Bug aus #471 nicht erkennen. Klasse und Fixtures (262 Zeilen) sind ersatzlos entfernt. Neu als Lehre daraus: `tests/test_issue_471_evidence_honesty.py` verbietet `check_expected()` außerhalb von `tests/evals/` (dort gibt es keine Modellausgabe zu bewerten, nur eigene Fixtures gegen eigene Erwartungen) und verbietet im CHANGELOG die Behauptung eines ausgeführten Belegs für Komponenten, die `docs/evals/STRATEGY.md` als `structural` führt; `TestEvalContrastPromptsAreWired` in `tests/test_topic_brainstorm.py` hält die Verdrahtung der Prompts fest. **Damit ausdrücklich nicht behauptet:** dass das Modell bei einer realen Anfrage von sich aus fachlich passende Themen entwirft. Das bleibt ohne API-Budget ungemessen — AC1 ist mechanisch erfüllt (kein Fixed-Set, keine stille Feld-Normalisierung, Kandidaten werden durchgereicht) und inhaltlich unbelegt.
- **Die Fach-Kontrast-Evals von `topic-brainstorm` konnten den Bug gar nicht erkennen (#471, zweite Fix-Runde zu PR #484):** Alle fünf Quality-Prompts forderten ausschließlich Wörter, die wörtlich in ihrem eigenen `input` stehen — `tb-04` verlangte u. a. „Maschinenbau" und „Fertigung", beides Teil der Frage. Spielt man die reproduzierte Ausgabe des in #471 behobenen Bugs ein (die fünf Cyber-Security-Themen der gelöschten `_TOPIC_DB`, dazu ein Satz „Für dein Maschinenbau-Studium …" und die Rubrik-Tabelle), meldeten `tb-01`, `tb-03`, `tb-04` und `tb-05` PASS: mit gesetztem `ANTHROPIC_API_KEY` hätten diese Prompts den Regressionsfall als bestanden gemeldet, und ein Echo-Kriterium trennt grundsätzlich nicht zwischen angewandtem Skill und bloß aufgegriffener Frage — liefe die Komponente je in `mode: "both"`, bestünden beide Arme, sodass der in `evals/SCHEMA.md` geforderte Baseline-Gap (≥ 20 pp) nicht entstehen könnte. `check_expected()` kennt deshalb jetzt ein optionales `expected.forbidden` (Regex-Negativkontrolle: trifft sie, ist der Case FAIL, auch bei erfülltem Positivkriterium); `tb-01`–`tb-03` fordern die Rubrik-Begriffe, die erst die Skill-Anwendung erzeugt (Feasibility/Novelty/Career-Fit), `tb-04`/`tb-05` je fachspezifisches Vokabular (Pulverbett, Schichtdicke, Prozessparameter … bzw. CSRD, Berichterstattung, Kennzahl …) plus `forbidden` gegen genau die gelöschte Themenliste. Zwei Guards halten das fest: `TestEvalCriteriaAreDiscriminative` in `tests/test_topic_brainstorm.py` (kein gefordertes Marker-Wort darf im eigenen Prompt stehen; die Kontrast-Prompts brauchen eine Negativkontrolle, die alle fünf Alt-Titel erfasst) und `tests/evals/test_topic_brainstorm_criteria.py` (die alten Kriterien ließen den Regressionsfall passieren — als Ausgangsbefund eingefroren —, die neuen lassen ihn durchfallen, während eine fachlich passende Antwort weiterhin besteht). **Weiterhin ungemessen:** ob das Modell bei einer realen Anfrage von sich aus fachlich passende Themen entwirft. Diese Runde repariert das Messinstrument, nicht die Messung — die bleibt an einen API-Key gebunden (Issue #55 bleibt geschlossen, kein Budget angefordert).
