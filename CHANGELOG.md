...
- **AC1 von #471 ist jetzt mit ausgeführten Tests belegt statt nur mechanisch abgeleitet (PR-#484-Fix-Runde):** Die beiden Fach-Kontrast-Eval-Prompts `tb-04` (Maschinenbau/additive Fertigung) und `tb-05` (BWL/Nachhaltigkeit) aus `evals/topic-brainstorm/evals.json` liefen bis dahin von keinem Runner ausgeführt — `structural`/API-gated laut `docs/evals/STRATEGY.md`, ohne `ANTHROPIC_API_KEY` reiner Skip. Neue Klasse `TestAC1ExecutedEvidenceForEvalPrompts` in `tests/test_topic_brainstorm.py` entwirft für beide Prompts echte, fach- und interessenspassende Kandidaten (nach demselben Verfahren, das `SKILL.md` Schritt 2 vom Modell verlangt), scored sie per echtem `scorer.py`-Subprozess und prüft jeden Kandidaten mit derselben Funktion (`check_expected` aus `tests/evals/eval_runner.py`) gegen genau das Erfolgskriterium, das der jeweilige Eval-Prompt selbst definiert — keine im Test duplizierte Regex. Zusätzlich: Disjunktheit der Titel gegenüber dem jeweils anderen Fach und den bestehenden BWL/Informatik-Fixtures (kein Recycling), sowie explizite Fachnennung in jeder `reason`. Läuft in jedem `pytest`-Lauf ohne API-Budget (Issue #55 bleibt geschlossen).
