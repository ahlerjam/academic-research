...
set -u
# Zielrepo-Anpassung: Ohne Argument die DEPLOYTE Datei testen, nicht das
# Template. `templates/hooks/` existiert nur im flowkit-Repo selbst; hier
# laeuft der Harness gegen die Datei, die Claude Code tatsaechlich ausfuehrt
# (#343). AGENTS.md und das CI-Gate rufen ihn ohne Argument auf.
REPO_ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
SCRIPT_ARG="${1:-$REPO_ROOT/.claude/hooks/pretooluse-blocker.sh}"
TMP=$(mktemp)
if [ -f "$SCRIPT_ARG" ]; then
  cp "$SCRIPT_ARG" "$TMP"
else
  sed -e 's/main|master/main|master/' -e 's/override-claude-review/override-claude-review/' \
